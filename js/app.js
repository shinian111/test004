class FaultTreeApp {
  constructor() {
    ...
    this.globalNotes = []; // 继承的注意事项栈
    this.loadedNodes = new Map(); // 缓存已加载的JSON节点
    this.imagePreloader = new Set(); // 图片预加载记录
  ...
  async fetchJSON(path) {
    const res = await fetch(path + '?t=' + new Date().getTime()); // 防缓存
  ...
  updateInheritedNotes() {
    ...
    while (tempPath.length > 0) {
      ...
      if (data && data.notes) {
        notes.unshift(data.notes); // 从上到下顺序
      }
    }
  ...
  preloadImagesInContent(item) {
    if (!item.measures) return;
    const imgRegex = /<image[^>]+src=['"]([^'"]+)['"]/g;
    let match;
    while ((match = imgRegex.exec(item.measures.join(' ')))) {
      const src = match[1];
  ...
  showError(msg) {
    this.contentArea.innerHTML = `<p style="color:red;">❌ ${msg}</p>`;
  }
}
